...
def extract_plcpcd(clx: bytes) -> bytes:
    ...
    while i < len(clx):
        tag = clx[i]
        i += 1
        if tag == 0x01:  # Prc skip
            if i + 2 > len(clx):
                raise ValueError("잘못된 Clx: Prc 헤더가 짧음")
            cb = struct.unpack_from("<H", clx, i)[0] #cbGrpprl 2바이트 읽기
            i += 2 + cb
        elif tag == 0x02:  # Pcdt
            if i + 4 > len(clx):
            ...
            return clx[i:i+lcb]  #PlcPcd 크기 반환
        else:
...
def parse_plcpcd(plcpcd: bytes):
    size = len(plcpcd)
    if (size - 4) % 12 != 0:
        raise ValueError("PlcPcd 길이 형식 불일치")
    n = (size - 4) // 12  # size = 4*(n+1) + 8*n (n은 조각 개수)
    
    # aCp 배열 읽기 (4바이트 정수 배열)
    acp = [struct.unpack_from("<I", plcpcd, 4*i)[0] for i in range(n+1)]
    
    #PCD 배열 시작 위치 (8바이트 배열)
    pcd_off = 4 * (n+1)
    
    pieces = []
    for k in range(n):
        pcd_bytes = plcpcd[pcd_off + 8*k : pcd_off + 8*(k+1)] #PCD = 8byte, aCp 다음 위치부터 PCD 배열 시작.
        flags = struct.unpack_from("<H", pcd_bytes, 0)[0] #앞 2바이트 -> flag
        ...
        cp_start = acp[k]
        cp_end   = acp[k+1]
        char_count = cp_end - cp_start  #해당 piece가 담당하는 문서내의 문자 수
        byte_count = char_count if fCompressed else char_count * 2  #압축일경우 1바이트로, 아니면 2바이트로 바이트계산 
...
def extract_full_text(word_data: bytes, pieces):
    ...
    for i, p in enumerate(pieces):
        ...
        if end_pos > len(word_data):
            print(f"조각이 WordDocument 범위를 벗어남")
            continue
        ...
        text = decode_piece(chunk, p["fCompressed"]) #fCompressed 값에 맞게 디코딩 처리
        debug_text = text.encode('unicode_escape').decode('ascii') #escape처리 
...
# 추출 텍스트 내 특정 문자열을 동일 길이의 '*'로 치환 후, WordDocument 스트림에 반영
target_text = "함근희"
